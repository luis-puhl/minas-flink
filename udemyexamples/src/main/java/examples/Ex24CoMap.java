...
public class Ex24CoMap {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStream<Tuple2<String, Double>> production = source
                ...
                .map(Ex24CoMap::mainMap);
        DataStream<Tuple2<String, Double>> hours = source
                .filter(value -> value.f0.equals(HOURS_KEY))
                .map(Ex24CoMap::mainMap);

        // sum numbers
        DataStream<Tuple2<String, Double>> coMapStream = production.connect(hours)
                .map(new ParseRow());
        coMapStream.print();
        coMapStream.writeAsText("out", FileSystem.WriteMode.OVERWRITE);

        env.execute("Window Co-Group");
    }

    private static class ParseRow implements CoMapFunction<Tuple2<String, Double>, Tuple2<String, Double>, Tuple2<String, Double>> {
        @Override
        public Tuple2<String, Double> map1(Tuple2<String, Double> value) throws Exception {
            return Tuple2.of(value.f0 + " produced", value.f1);
        }

        @Override
        public Tuple2<String, Double> map2(Tuple2<String, Double> value) throws Exception {
            return Tuple2.of(value.f0 + " worked", value.f1);
        }
    }
}
