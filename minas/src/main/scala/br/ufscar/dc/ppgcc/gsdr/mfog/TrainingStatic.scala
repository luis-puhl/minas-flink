...
object TrainingStatic {
  ...
  def main(args: Array[String]): Unit = {
    ...
    LOG.info(s"modelSeq = ${modelSeq.size} ${(System.currentTimeMillis() - start) * 10e-4}s => ${modelSeq.head.json.toString}")
    ...
    LOG.info(s"sent = ${modelSeq.size} in ${(System.currentTimeMillis() - start) * 10e-4}s")
  }
}
